...
async function checkAA13537() {
  console.log('=== AA13537のデータ確認 ===\n');

  // sellersテーブルから取得
  const { data: seller, error } = await supabase
    .from('sellers')
    .select('*')
    .eq('seller_number', 'AA13537')
    .single();
  ...
  if (!seller) {
    console.log('❌ AA13537が見つかりません');
    return;
  }

  console.log('✅ AA13537のデータ:');
  console.log('売主番号:', seller.seller_number);
  console.log('営担（visit_assignee）:', seller.visit_assignee || '（空欄）');
  console.log('営担の型:', typeof seller.visit_assignee);
  ...
  if (hasVisitAssignee) {
    ...
    console.log('   理由: 営担が空欄');
    console.log('   → 「その他(Y)」に表示されるべきではない');
  }
}

checkAA13537().catch(console.error);
