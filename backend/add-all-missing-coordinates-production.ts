// 本番環境：座標がない全公開前物件に座標を一括追加するスクリプト
...
async function addAllMissingCoordinates() {
  console.log('🌐 PRODUCTION: Adding coordinates for all pre-publish properties without coordinates...\n');
  ...
  try {
    ...
    if (successCount > 0) {
      ...
      console.log('  Pre-publish properties will now display with orange markers on the map.');
    }
  } catch (error) {
