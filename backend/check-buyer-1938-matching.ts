...
async function checkBuyer1938Matching() {
  console.log('=== 買主1938とAA13249のマッチング確認 ===\n');
  ...
  const { data: buyer, error: buyerError } = await supabase
    .from('buyers')
    .select('*')
    .eq('buyer_number', '1938')
    .single();

  if (buyerError || !buyer) {
    console.error('買主1938が見つかりません:', buyerError);
    return;
  }
...
checkBuyer1938Matching().catch(console.error);
