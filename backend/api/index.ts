...
// デバッグ用：コメント同期の詳細ログを返すエンドポイント
app.post('/api/admin/debug-sync-comments/:propertyNumber', async (req, res) => {
  const { propertyNumber } = req.params;
  const logs: string[] = [];
  ...
  try {
    ...
    if (propertyError || !property) {
      return res.json({ success: false, error: 'Property not found', logs });
    }
    logs.push(`property_type: ${property.property_type}`);
    
    const englishPropertyType = convertPropertyTypeToEnglish(property.property_type);
    logs.push(`englishPropertyType: ${englishPropertyType}`);
    
    if (!englishPropertyType) {
      return res.json({ success: false, error: `Invalid property type: ${property.property_type}`, logs });
    }
    
    // GyomuListServiceでスプレッドシートIDを取得
    ...
    logs.push(`gyomuData: ${JSON.stringify(gyomuData)}`);
    
    if (!gyomuData || !gyomuData.spreadsheetUrl) {
      return res.json({ success: false, error: 'Spreadsheet URL not found in gyomu list', logs });
    }
    
    // URLからスプレッドシートIDを抽出
    const match = gyomuData.spreadsheetUrl.match(/\/d\/([a-zA-Z0-9-_]+)/);
    const spreadsheetId = match ? match[1] : null;
    logs.push(`spreadsheetId: ${spreadsheetId}`);
    
    if (!spreadsheetId) {
      return res.json({ success: false, error: 'Could not extract spreadsheet ID from URL', logs });
    }
    
    // AthomeSheetSyncServiceでコメントを取得
    const athomeSheetSyncService = new AthomeSheetSyncService();
    try {
      const comments = await athomeSheetSyncService.fetchCommentsFromAthomeSheet(spreadsheetId, englishPropertyType);
      logs.push(`favoriteComment: ${comments.favoriteComment}`);
      logs.push(`recommendedComments count: ${comments.recommendedComments.length}`);
      logs.push(`panoramaUrl: ${comments.panoramaUrl}`);
      return res.json({ success: true, comments, logs });
    } catch (fetchError: any) {
      logs.push(`fetchError: ${fetchError.message}`);
      return res.json({ success: false, error: fetchError.message, logs });
    }
