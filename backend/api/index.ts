...
app.get('/api/public/properties/:propertyIdentifier', async (req, res) => {
  try {
    ...
    // property_detailsテーブルからおすすめコメントなどを取得
    const { data: propertyDetails } = await supabase
      .from('property_details')
      .select('recommended_comments, favorite_comment, athome_data, property_about')
      .eq('property_number', property.property_number)
      .single();

    console.log(`📋 Property details:`, {
      has_recommended_comments: !!propertyDetails?.recommended_comments,
      has_favorite_comment: !!propertyDetails?.favorite_comment,
      has_athome_data: !!propertyDetails?.athome_data,
      has_property_about: !!propertyDetails?.property_about
    });
    ...
    res.json({ 
      ...
      property: {
        ...property,
        images,
        recommendedComments: propertyDetails?.recommended_comments || null,
        favoriteComment: propertyDetails?.favorite_comment || null,
        athomeData: propertyDetails?.athome_data || null,
        propertyAbout: propertyDetails?.property_about || null
      }
    });
  } catch (error: any) {
...
app.post('/api/public/inquiries', async (req, res) => {
  try {
    console.log('[Inquiry] Received inquiry request:', {
      body: req.body,
    ...
    // バリデーション
    const { property_id, propertyId, name, email, phone, message } = req.body;
    
    // property_idまたはpropertyIdのいずれかが必要
    const propertyIdOrNumber = property_id || propertyId;
    
    if (!propertyIdOrNumber) {
      return res.status(400).json({
        success: false,
        message: '物件IDまたは物件番号を指定してください',
        errors: [{ field: 'propertyId', message: '物件IDまたは物件番号を指定してください' }]
      });
    }
    ...
    if (!name || name.length === 0 || name.length > 100) {
      return res.status(400).json({
        ...
        errors: [{ field: 'name', message: '名前は1〜100文字で入力してください' }]
      });
    }
    ...
    if (!phone || phone.length < 10 || phone.length > 15 || !/^[0-9\-+() ]+$/.test(phone)) {
      return res.status(400).json({
        ...
        errors: [{ field: 'phone', message: '電話番号は10〜15文字の数字、ハイフン、括弧のみ使用できます' }]
      });
    }
    ...
    if (!message || message.length === 0 || message.length > 2000) {
      return res.status(400).json({
        ...
        errors: [{ field: 'message', message: 'お問い合わせ内容は1〜2000文字で入力してください' }]
      });
    }

    console.log('[Inquiry] Property ID or Number:', propertyIdOrNumber);

    // UUIDかどうかをチェック
    const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(propertyIdOrNumber);

    console.log('[Inquiry] Is UUID:', isUUID);
    ...
    if (isUUID) {
      console.log('[Inquiry] Searching by UUID:', propertyIdOrNumber);
      query = query.eq('id', propertyIdOrNumber);
    } else {
      console.log('[Inquiry] Searching by property_number:', propertyIdOrNumber);
      query = query.eq('property_number', propertyIdOrNumber);
    }

    const { data: property, error: propertyError } = await query.single();

    console.log('[Inquiry] Property search result:', { property, propertyError });

    if (propertyError || !property) {
      console.error('[Inquiry] Property not found or error:', propertyError);
      return res.status(404).json({
        success: false,
    ...
    try {
      ...
      console.log('[Inquiry] Reading all rows from buyer sheet...');
      const allRows = await sheetsClient.readAll();
      console.log('[Inquiry] Read rows:', allRows ? allRows.length : 'undefined');
      
      if (!allRows || !Array.isArray(allRows)) {
        console.error('[Inquiry] Failed to read buyer list rows');
      ...
      const receptionDate = new Date().toLocaleDateString('ja-JP');
      
      // ATBB状況に基づいて問合せ元を判定
      let inquirySource = 'サイト';
      ...
      console.log('[Inquiry] Writing to row:', targetRowIndex);
      await sheetsClient.updateRow(targetRowIndex, rowData);
      console.log('[Inquiry] Row written successfully');
      ...
      console.error('[Inquiry] Failed to sync to buyer sheet:', syncError);
      console.error('[Inquiry] Sync error details:', {
    ...
    console.error('[Inquiry] Error details:', {
      message: error.message,
      stack: error.stack,
      code: error.code,
    });
