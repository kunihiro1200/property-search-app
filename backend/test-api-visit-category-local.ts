/**
 * ローカルAPIエンドポイントの訪問予定/訪問済みデータを確認
 */

async function main() {
  console.log('=== ローカルAPIエンドポイントの訪問予定/訪問済みデータ確認 ===');
  console.log('');
  ...
  try {
    ...
    if (!visitScheduledResponse.ok) {
      console.error('APIエラー:', visitScheduledResponse.status, visitScheduledResponse.statusText);
    ...
    if (!visitCompletedResponse.ok) {
      console.error('APIエラー:', visitCompletedResponse.status, visitCompletedResponse.statusText);
    ...
    const allSellers = [...(visitScheduledData.data || []), ...(visitCompletedData.data || [])];
