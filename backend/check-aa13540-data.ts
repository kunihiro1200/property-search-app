...
async function checkAA13540() {
  console.log('=== AA13540のデータ確認 ===\n');

  // sellersテーブルから取得
  const { data: seller, error } = await supabase
    .from('sellers')
    .select('*')
    .eq('seller_number', 'AA13540')
    .single();
  ...
  if (!seller) {
    console.log('❌ AA13540が見つかりません');
    return;
  }

  console.log('✅ AA13540のデータ:');
...
checkAA13540().catch(console.error);
