...
async function findBuyerWithApartment() {
  console.log('マンションの買主を検索中...\n');

  // マンションの物件を取得
  const { data: properties, error: propertiesError } = await supabase
    .from('property_listings')
    .select('property_number, address, property_type')
    .ilike('property_type', '%マンション%')
    .limit(10);
  ...
  if (!properties || properties.length === 0) {
    console.log('マンションの物件が見つかりません');
    return;
  }

  console.log('マンションの物件:', properties.length, '件');
