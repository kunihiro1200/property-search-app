# コメントデータ同期失敗の防止策

**作成日**: 2026年1月28日  
**目的**: スプレッドシートにデータが存在するのに同期できない問題を防ぐ

---

## 🎯 問題の本質

**問題**: スプレッドシートにコメントデータが確実に存在するのに、データベースに同期されていない

**影響**:
- ユーザーは公開物件サイトでコメントを見ることができない
- 物件の魅力が伝わらない
- ビジネスに悪影響

---

## 🔍 根本原因

### 原因1: `EnhancedAutoSyncService`が空のレコードを作成していた

**問題のコード**（修正前）:
```typescript
// 一時的な解決策: property_detailsに空のレコードを作成
const { error: insertError } = await this.supabase
  .from('property_details')
  .insert({
    property_number: propertyNumber,
    property_about: null,              // ← スプレッドシートから取得していない
    recommended_comments: null,        // ← スプレッドシートから取得していない
    athome_data: null,                 // ← スプレッドシートから取得していない
    favorite_comment: null,            // ← スプレッドシートから取得していない
  });
```

**なぜ問題なのか**:
- スプレッドシートからデータを取得していない
- 空のレコードを作成するだけ
- ユーザーはコメントを見ることができない

### 原因2: 同期タイミングの問題

**問題**:
- 新規物件が`property_listings`に追加される
- `EnhancedAutoSyncService`が実行される
- 空のレコードが作成される
- その後、誰もスプレッドシートからデータを取得しない

**結果**:
- コメントデータが永遠に`null`のまま

---

## ✅ 実装した対策

### 対策1: `EnhancedAutoSyncService`の修正

**修正内容**: スプレッドシートからデータを取得するように変更

**修正後のコード**:
```typescript
// 物件情報を取得（物件種別が必要）
const { data: property, error: propertyError } = await this.supabase
  .from('property_listings')
  .select('property_type')
  .eq('property_number', propertyNumber)
  .single();

if (propertyError || !property) {
  console.error(`❌ ${propertyNumber}: Property not found in property_listings`);
  failed++;
  continue;
}

// AthomeSheetSyncServiceを使用してスプレッドシートからコメントデータを取得
const { AthomeSheetSyncService } = await import('./AthomeSheetSyncService');
const athomeSheetSyncService = new AthomeSheetSyncService();

const syncSuccess = await athomeSheetSyncService.syncPropertyComments(
  propertyNumber,
  property.property_type as 'land' | 'detached_house' | 'apartment'
);

if (syncSuccess) {
  console.log(`✅ ${propertyNumber}: Synced comments from spreadsheet`);
  synced++;
} else {
  console.error(`❌ ${propertyNumber}: Failed to sync comments from spreadsheet`);
  failed++;
}
```

**効果**:
- ✅ 新規物件が追加されたとき、自動的にスプレッドシートからコメントデータを取得
- ✅ 空のレコードを作成しない
- ✅ ユーザーはすぐにコメントを見ることができる

### 対策2: `/complete`エンドポイントの自動同期

**実装内容**: コメントデータが`null`の場合、自動的に同期

**コード**:
```typescript
// コメントデータがnullの場合、Athomeシートから自動同期
const needsSync = !details.favorite_comment && !details.recommended_comments;
if (needsSync) {
  console.log(`[Complete API] Comment data is null, syncing from Athome sheet...`);
  try {
    const athomeSheetSyncService = new AthomeSheetSyncService();
    const syncSuccess = await athomeSheetSyncService.syncPropertyComments(
      property.property_number,
      property.property_type as 'land' | 'detached_house' | 'apartment'
    );
    
    if (syncSuccess) {
      console.log(`[Complete API] Successfully synced comments from Athome sheet`);
      // 同期後のデータを再取得
      const updatedDetails = await propertyDetailsService.getPropertyDetails(property.property_number);
      return updatedDetails;
    }
  } catch (syncError: any) {
    console.error(`[Complete API] Error syncing comments:`, syncError.message);
  }
}
```

**効果**:
- ✅ 既存の物件でコメントデータが`null`の場合、初回アクセス時に自動同期
- ✅ ユーザーは次回アクセス時にコメントを見ることができる
- ✅ 手動で同期する必要がない

---

## 🔄 同期フロー（修正後）

### フロー1: 新規物件追加時

```
1. 新規物件がproperty_listingsに追加される
2. EnhancedAutoSyncServiceが実行される
3. property_detailsに物件が存在しないことを検出
4. AthomeSheetSyncServiceを使用してスプレッドシートからコメントデータを取得
5. property_detailsに保存（データあり）
6. ユーザーは公開物件サイトでコメントを見ることができる
```

### フロー2: 既存物件でコメントがnullの場合

```
1. ユーザーが公開物件詳細画面にアクセス
2. /completeエンドポイントが呼ばれる
3. property_detailsを確認 → コメントデータがnull
4. AthomeSheetSyncServiceを使用してスプレッドシートからコメントデータを取得
5. property_detailsを更新（データあり）
6. ユーザーはコメントを見ることができる
```

---

## 🛡️ 多層防御戦略

### 第1層: `EnhancedAutoSyncService`

**タイミング**: 新規物件追加時（自動、15分ごと）  
**役割**: スプレッドシートからコメントデータを取得して保存  
**効果**: 新規物件のコメントデータが確実に同期される  
**リトライ**: 3回まで自動リトライ（1秒間隔）

### 第2層: `/complete`エンドポイントの自動同期

**タイミング**: 公開物件詳細画面の初回アクセス時（自動）  
**役割**: コメントデータが`null`の場合、スプレッドシートから取得  
**効果**: 既存物件でコメントデータが`null`の場合も自動修正  
**リトライ**: 3回まで自動リトライ（1秒間隔）

### 第3層: 手動同期エンドポイント ✅ 実装完了

**タイミング**: 管理者が手動で実行  
**役割**: 特定の物件または全物件のコメントデータを再同期  
**効果**: 問題が発生した場合の緊急対応  
**エンドポイント**:
- **単一物件**: `POST /api/admin/sync-comments/:propertyNumber`
- **一括同期**: `POST /api/admin/sync-comments-batch` (body: `{ propertyNumbers: ['AA13453', 'AA12398'] }`)

**使用例**:
```bash
# 単一物件の同期
curl -X POST http://localhost:3000/api/admin/sync-comments/AA13453

# 一括同期
curl -X POST http://localhost:3000/api/admin/sync-comments-batch \
  -H "Content-Type: application/json" \
  -d '{"propertyNumbers": ["AA13453", "AA12398", "CC100"]}'
```

### 第4層: 定期監視スクリプト ✅ 実装完了

**タイミング**: 定期的に実行（推奨: 1日1回）  
**役割**: コメントデータが`null`の物件を検出してアラート  
**効果**: 問題の早期発見  
**実行方法**:
```bash
npx ts-node backend/monitor-comment-sync-status.ts
```

**アラート条件**:
- コメントデータが`null`の物件が10件以上
- 同期成功率が90%未満

---

## 📊 監視とアラート

### 監視すべき指標

1. **コメントデータがnullの物件数**
   ```sql
   SELECT COUNT(*) 
   FROM property_details 
   WHERE favorite_comment IS NULL 
     AND recommended_comments IS NULL;
   ```

2. **同期失敗率**
   - `EnhancedAutoSyncService`のログを確認
   - 失敗した物件をリスト化

3. **自動同期の実行回数**
   - `/complete`エンドポイントで自動同期が実行された回数
   - 頻繁に実行される場合は、第1層に問題がある

### アラート設定

**条件1**: コメントデータがnullの物件が10件以上
- → `EnhancedAutoSyncService`に問題がある可能性

**条件2**: 同期失敗率が10%以上
- → スプレッドシートのアクセス権限やAPI制限の問題

**条件3**: 自動同期が1日に100回以上実行される
- → 第1層が正常に動作していない

---

## 🔧 トラブルシューティング

### 問題1: コメントデータが同期されない

**確認事項**:
1. スプレッドシートにデータが存在するか？
   ```bash
   npx ts-node backend/get-aa13453-comments.ts
   ```

2. 業務リストに物件が存在するか？
   ```bash
   npx ts-node backend/check-aa13453-in-gyomu-list.ts
   ```

3. 物件種別が正しいか？
   ```sql
   SELECT property_number, property_type 
   FROM property_listings 
   WHERE property_number = 'AA13453';
   ```

**解決策**:
- スプレッドシートにデータがない → スプレッドシートを更新
- 業務リストに物件がない → 業務リストに追加
- 物件種別が間違っている → `property_listings`を更新

### 問題2: 同期が遅い

**原因**:
- Google Sheets APIのレート制限
- スプレッドシートのサイズが大きい

**解決策**:
- バッチ処理のサイズを調整（現在: 10件/バッチ）
- キャッシュを活用（現在: 5分間）
- 並列処理を制限

### 問題3: 同期エラーが頻発

**原因**:
- Google Sheets APIの認証エラー
- スプレッドシートのアクセス権限がない

**解決策**:
- サービスアカウントの認証情報を確認
- スプレッドシートの共有設定を確認

---

## ✅ チェックリスト

### 新規物件追加時

- [ ] `property_listings`に物件を追加
- [ ] 業務リストに物件を追加（`スプシURL`を設定）
- [ ] 個別物件スプレッドシートの`athome`シートにコメントデータを入力
- [ ] `EnhancedAutoSyncService`が自動実行される（15分ごと）
- [ ] `property_details`にコメントデータが保存されたことを確認

### 既存物件でコメントがnullの場合

- [ ] 公開物件詳細画面にアクセス
- [ ] `/complete`エンドポイントが自動同期を実行
- [ ] `property_details`にコメントデータが保存されたことを確認
- [ ] 次回アクセス時にコメントが表示されることを確認

---

## 📞 Kiroへの伝え方

**📖 詳細ガイド**: `KIRO_CALLOUT_GUIDE.md` を参照してください。

### 基本の呼びかけ（推奨）

```
【物件番号】のコメントが同期されていない。
スプシにはデータがある。
COMMENT_SYNC_FAILURE_PREVENTION.mdを参照して対応して。
```

**例**:
```
AA13453のコメントが同期されていない。
スプシにはデータがある。
COMMENT_SYNC_FAILURE_PREVENTION.mdを参照して対応して。
```

### 複数物件の場合

```
以下の物件のコメントが同期されていない。
スプシにはデータがある。
COMMENT_SYNC_FAILURE_PREVENTION.mdを参照して一括同期して。

物件番号:
- AA13453
- AA12398
- CC100
```

### 緊急対応が必要な場合

```
【物件番号】のコメントが同期されていない。緊急対応が必要。
手動同期エンドポイントを実行して。
```

### 最短の呼びかけ

```
【物件番号】のコメント同期失敗。
COMMENT_SYNC_FAILURE_PREVENTION.md参照。
```

---

## 🎯 まとめ

### 実装した対策

1. ✅ `EnhancedAutoSyncService`を修正（スプレッドシートからデータを取得）
2. ✅ `/complete`エンドポイントに自動同期を追加（セーフティネット）
3. ✅ 多層防御戦略を実装（第1層 + 第2層）

### 効果

- ✅ 新規物件のコメントデータが確実に同期される
- ✅ 既存物件でコメントがnullの場合も自動修正される
- ✅ ユーザーは常にコメントを見ることができる

### 今後の改善

- [x] 手動同期エンドポイントの実装（第3層） ✅ 完了
- [x] 定期監視スクリプトの実装（第4層） ✅ 完了
- [x] 同期失敗時の自動リトライ機能 ✅ 完了
- [ ] Slackアラート連携（監視スクリプトから通知）
- [ ] 同期失敗の詳細ログ記録（データベーステーブル）
- [ ] ダッシュボードでの可視化（同期成功率、失敗物件リスト）

---

**最終更新日**: 2026年1月28日  
**ステータス**: ✅ 対策実装完了


---

## 📖 使用方法ガイド

### 1. 定期監視（推奨: 毎日実行）

```bash
# ローカル環境
npx ts-node backend/monitor-comment-sync-status.ts

# 本番環境（Vercelにデプロイ後）
# Vercel Cronジョブとして設定することを推奨
```

**期待される出力**:
```
🔍 Monitoring comment sync status...

📊 Total properties in property_details: 150

⚠️  Properties with null comments: 2

📋 List of properties with null comments:
  1. AA13453
     - favorite_comment: ❌ null
     - recommended_comments: ❌ null
     - property_about: ✅

📈 Sync success rate: 98.67%

✅ All systems normal
```

### 2. 手動同期（問題発生時）

#### 単一物件の同期

```bash
# ローカル環境
curl -X POST http://localhost:3000/api/admin/sync-comments/AA13453

# 本番環境
curl -X POST https://your-domain.vercel.app/api/admin/sync-comments/AA13453
```

**期待される出力**:
```json
{
  "success": true,
  "message": "Successfully synced comments for AA13453",
  "propertyNumber": "AA13453"
}
```

#### 一括同期

```bash
# ローカル環境
curl -X POST http://localhost:3000/api/admin/sync-comments-batch \
  -H "Content-Type: application/json" \
  -d '{"propertyNumbers": ["AA13453", "AA12398", "CC100"]}'

# 本番環境
curl -X POST https://your-domain.vercel.app/api/admin/sync-comments-batch \
  -H "Content-Type: application/json" \
  -d '{"propertyNumbers": ["AA13453", "AA12398", "CC100"]}'
```

**期待される出力**:
```json
{
  "success": true,
  "message": "Synced 3/3 properties",
  "results": {
    "success": 3,
    "failed": 0,
    "details": [
      { "propertyNumber": "AA13453", "success": true },
      { "propertyNumber": "AA12398", "success": true },
      { "propertyNumber": "CC100", "success": true }
    ]
  }
}
```

### 3. 自動リトライの動作確認

```bash
# ログを確認
# ローカル環境
npm run dev

# 本番環境
vercel logs
```

**期待されるログ**:
```
[AthomeSheetSyncService] Syncing comments for AA13453 (land) - Attempt 1/3
[AthomeSheetSyncService] ❌ Attempt 1/3 failed for AA13453: Network error
[AthomeSheetSyncService] Retrying in 1000ms...
[AthomeSheetSyncService] Syncing comments for AA13453 (land) - Attempt 2/3
[AthomeSheetSyncService] ✅ Successfully synced comments for AA13453 (attempt 2)
```

---

## 🔔 アラート設定（推奨）

### Vercel Cronジョブの設定

`vercel.json`に以下を追加：

```json
{
  "crons": [
    {
      "path": "/api/cron/monitor-comments",
      "schedule": "0 9 * * *"
    }
  ]
}
```

### 監視エンドポイントの作成

`backend/api/cron/monitor-comments.ts`:

```typescript
import type { VercelRequest, VercelResponse } from '@vercel/node';
import { createClient } from '@supabase/supabase-js';

export default async function handler(req: VercelRequest, res: VercelResponse) {
  const supabase = createClient(
    process.env.SUPABASE_URL!,
    process.env.SUPABASE_SERVICE_KEY!
  );

  // コメントデータがnullの物件を取得
  const { data: nullCommentProperties, error } = await supabase
    .from('property_details')
    .select('property_number')
    .or('favorite_comment.is.null,recommended_comments.is.null');

  if (error) {
    return res.status(500).json({ error: error.message });
  }

  const count = nullCommentProperties?.length || 0;

  // アラート条件
  if (count >= 10) {
    // Slackに通知（実装例）
    // await sendSlackAlert(`⚠️ ${count} properties have null comments`);
    
    return res.json({
      alert: true,
      message: `${count} properties need attention`,
      properties: nullCommentProperties
    });
  }

  return res.json({
    alert: false,
    message: 'All systems normal',
    count
  });
}
```

---

**最終更新日**: 2026年1月28日  
**ステータス**: ✅ 完全実装完了
