...
async function testPriceRange900() {
  console.log('予算900万円のテストを実行中...\n');

  // 問合せ時ヒアリングを「予算：900万円」に変更
  const inquiryHearing = '希望時期：2年以内\n駐車場希望台数：2台\n予算：900万円';

  console.log('1. 問合せ時ヒアリングを更新中...');
  console.log('  予算: 900万円');
  ...
  // 検証
  console.log('\n検証結果:');
  if (after.price_range_land === '~1900万円' && !after.price_range_house && !after.price_range_apartment) {
    console.log('✅ 価格帯が正しく「~1900万円」に設定されました');
  } else {
    console.log('❌ 価格帯の反映が正しくありません');
    console.log('  期待値: 価格帯（土地）= ~1900万円、価格帯（戸建）= 未設定、価格帯（マンション）= 未設定');
