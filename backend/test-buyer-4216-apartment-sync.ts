...
async function testBuyer4216ApartmentSync() {
  console.log('買主番号4216のマンションテストを実行中...\n');

  // desired_property_typeを「マンション」に変更
  console.log('1. desired_property_typeを「マンション」に変更中...');
  await supabase
    .from('buyers')
    .update({ desired_property_type: 'マンション' })
  ...
  // 検証
  console.log('\n検証結果:');
  if (after.price_range_apartment === '3000万円台' && !after.price_range_house && !after.price_range_land) {
    console.log('✅ 価格帯が正しくマンションのみに反映されました');
  } else {
    console.log('❌ 価格帯の反映が正しくありません');
    console.log('  期待値: 価格帯（マンション）= 3000万円台、価格帯（戸建）= 未設定、価格帯（土地）= 未設定');
    console.log('  実際の値: 価格帯（マンション）=', after.price_range_apartment, '、価格帯（戸建）=', after.price_range_house, '、価格帯（土地）=', after.price_range_land);
  }
