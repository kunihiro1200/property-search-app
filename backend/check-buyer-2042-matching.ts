...
async function checkBuyer2042Matching() {
  console.log('=== 買主2042とAA13249のマッチング確認 ===\n');
  ...
  const { data: buyer, error: buyerError } = await supabase
    .from('buyers')
    .select('*')
    .eq('buyer_number', '2042')
    .single();

  if (buyerError || !buyer) {
    console.error('買主2042が見つかりません:', buyerError);
    return;
  }
  ...
  console.log('\n=== 結論 ===\n');
  if (!areaMatch) {
    console.log('買主2042が買主候補に含まれない理由: エリアが一致しません');
  } else if (!typeMatch) {
    console.log('買主2042が買主候補に含まれない理由: 種別が一致しません');
  } else if (!priceMatch) {
    console.log('買主2042が買主候補に含まれない理由: 価格帯が一致しません');
  } else {
    console.log('買主2042は全ての条件を満たしています。買主候補に含まれるはずです。');
  }
}
...
checkBuyer2042Matching().catch(console.error);
