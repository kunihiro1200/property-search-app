 ...
 * - limit=5000, offset=0, withCoordinates=true, skipImages=true
 */
...
describe('バグ条件1: fetchAllProperties の while ループが廃止されている（修正確認）', () => {
  ...
  test('fetchAllProperties のソースコードに limit=5000 の単一リクエストが存在する（修正済み）', () => {
    /**
     * 修正後の実装: limit=5000 の単一リクエストを使用
     */
    const sourceFilePath = path.resolve(
      __dirname,
      '../PublicPropertiesPage.tsx'
    ...
    // 修正後: limit=5000 が存在することを確認
    expect(sourceCode).toContain("limit: '5000'");
  });

  test('fetchAllProperties のソースコードに withCoordinates=true が存在する（修正済み）', () => {
    /**
     * 修正後の実装: withCoordinates=true パラメータを使用
     */
    const sourceFilePath = path.resolve(
      __dirname,
      '../PublicPropertiesPage.tsx'
    ...
    // 修正後: withCoordinates=true が存在することを確認
    expect(sourceCode).toContain("withCoordinates: 'true'");
  });
});
...
describe('バグ条件2: 修正後の fetchAllProperties は常に API を 1 回のみ呼び出す', () => {
  ...
  test('修正後のリクエストに withCoordinates=true と skipImages=true が含まれる', async () => {
    ...
    // リクエストURLに limit=5000 が含まれることを確認
    expect(result.requestUrls[0]).toContain('limit=5000');
  });
});
