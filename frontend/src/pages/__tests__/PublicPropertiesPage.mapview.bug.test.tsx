...
describe('バグ条件1: fetchAllProperties の while ループが廃止されている（修正確認）', () => {
  ...
  test('fetchAllProperties のソースコードに limit=500 の単一リクエストが存在する（修正済み）', () => {
    /**
     * 修正後の実装: limit=500 の単一リクエストを使用（座標付き物件は数百件程度）
     */
    const sourceFilePath = path.resolve(
      __dirname,
      '../PublicPropertiesPage.tsx'
    ...
    // 修正後: limit=500 が存在することを確認
    expect(sourceCode).toContain("limit: '500'");
  });
