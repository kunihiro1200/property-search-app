...
### 3-2. 「当日TEL（担当）」- 訪問予定/訪問済みのサブカテゴリー

**サイドバー表示**: 訪問予定(Y)または訪問済み(Y)の下にインデント表示
**色**: オレンジ（#ff5722）

**🚨 最重要：カテゴリーの排他性**

**訪問予定の当日TEL**と**訪問済みの当日TEL**は**完全に別のカテゴリ**です。

- **訪問予定(Y)の下の「当日TEL(Y)」** = 訪問予定 **かつ** 当日TEL
  - 条件: 営担=Y + 訪問日が今日以降 + 次電日が今日以前 + 状況に「追客中」が含まれる
  
- **訪問済み(Y)の下の「当日TEL(Y)」** = 訪問済み **かつ** 当日TEL
  - 条件: 営担=Y + 訪問日が昨日以前 + 次電日が今日以前 + 状況に「追客中」が含まれる

**一つのカテゴリに属したら、別のカテゴリには属さない**（排他的）

**条件**:
- **営担（visitAssignee）に入力がある**（「外す」以外）
- **次電日が今日以前**
- **状況（当社）に「追客中」が含まれる** ← **必須**

**表示位置**:
- 訪問日が今日以降の場合 → 訪問予定(イニシャル)のサブカテゴリーとして表示
- 訪問日が昨日以前の場合 → 訪問済み(イニシャル)のサブカテゴリーとして表示
- 訪問日がない場合 → 独立したカテゴリとして表示

**表示例**:
```
①訪問予定(U)
  └ 当日TEL(U)  ← サブカテゴリー（訪問予定かつ当日TEL）

②訪問済み(Y)
  └ 当日TEL(Y)  ← サブカテゴリー（訪問済みかつ当日TEL）
```

**クリック時の動作**:
- 「訪問予定(U)」の下の「└ 当日TEL(U)」をクリック → **訪問予定かつ当日TEL**の売主のみ表示
- 「訪問済み(Y)」の下の「└ 当日TEL(Y)」をクリック → **訪問済みかつ当日TEL**の売主のみ表示

**重要**: 
- 訪問予定/訪問済みの売主が次電日も今日以前の場合、サブカテゴリーとして表示される
- 一目で「訪問予定/訪問済みの中で、今日電話が必要な売主」がわかる
...
| コミュニケーション情報別カテゴリ | 追客中 + 次電日が今日以前 + コミュニケーション情報のいずれかに入力あり + **営担なし** | 紫 |
...
**更新履歴**:
- 2026年2月3日: 「当日TEL（内容）」を「コミュニケーション情報別カテゴリ」に変更（独立した本カテゴリとして扱う）
