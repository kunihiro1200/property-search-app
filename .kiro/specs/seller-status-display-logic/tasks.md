...
- [ ] 1.1 `sellerStatusUtils.ts`ファイルを作成
...
- [ ] 5.1 スプレッドシートのカラムマッピングを確認
  - 場所: `backend/src/services/SellerSyncService.ts`（または適切なファイル）
  - 確認事項:
    - D列（次電日）が`next_call_date`にマッピングされているか
    - AH列（状況（当社））が`situation_company`にマッピングされているか
